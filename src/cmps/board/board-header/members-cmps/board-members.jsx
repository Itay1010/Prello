...
export const BoardMembers = ({ board, onChangeMembers }) => {
    ...
    return <section className='board-members-header'>
        <section className='members-list'>
            ...
            />}
            {isMoreOpen && leftOvers.length > 0 && <MoreMembersModal members={leftOvers} />}
        </section>
        {
            leftOvers.length > 0 && <button className='extra-btn reset' onClick={ev => setMoreOpen(prevState => !prevState)}>
                {`+${leftOvers.length}`}
            </button>
        }
