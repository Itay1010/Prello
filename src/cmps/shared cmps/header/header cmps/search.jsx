...
export const Search = ({ minis, isSearchOpen }) => {
    ...
    return <div className={`search flex align-center ${isSearchOpen ? 'search-open' : ''}`}>
        ...
        <input ref={inputRef} onBlur={(ev) => ev.target.value = ''} type='text' placeholder='Search' name='search' onChange={handleSearchChange} autoComplete='off' />
        {minisSearched.search && filteredBoards.length !== 0 && <DynamicFilter boards={filteredBoards} setModal={onChooseBoard} />}
    </div>
}